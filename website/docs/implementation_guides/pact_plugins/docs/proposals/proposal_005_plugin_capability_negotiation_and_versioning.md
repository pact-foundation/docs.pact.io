...
The plugin declares to the driver which optional capabilities it supports. The driver must also advertise to the plugin which host capabilities it exposes (for example, host-provided matching from [009](/implementation_guides/pact_plugins/docs/proposals/proposal_009_host_provided_core_matching_and_generation)). Both directions flow through the `InitPlugin` handshake.
