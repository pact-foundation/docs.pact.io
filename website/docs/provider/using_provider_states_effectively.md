...
The purpose of contract testing is to ensure that the consumer and provider have a shared understanding of the messages that will pass between them. When verifying a pact, you are not only ensuring that the provider returns the expected data to the consumer — you are also verifying that the consumer is calling the API correctly.
...
How did this happen? The contract test passed, right?

The correct parameter for the alligator search was "firstname", not "name". During pact verification, the "name" query param was ignored, and the request was interpreted as `/search-alligators`, which returned all alligators — which happened to be just the one in the database at the time, called "Mary".

The test produced a false positive.
...
This solution takes cooperation on both sides of the contract. As described elsewhere, contracts should not be used to do functional testing — those tests belong in the provider's codebase. In this situation, unfortunately the only way to check that the right parameter name has been used is to check the results that are returned. Here is one solution.
